<template>
    <div>
        <q-btn size="sm" label="Create User" right-icon="person_add" color="negative" class="q-ma-sm" rounded @click="prompt = !prompt" />
        <q-table
            dense
            dark
            flat
                ...
                <q-tr :props="props">
                    ...
                    <q-td key="id" :props="props" style="text-align:center" width="5px">
                        <q-avatar size="24px" v-if="props.row.profile != ''">
                            <q-img circle :src="props.row.profile != '' ? props.row.profile : ''" />
                        </q-avatar>
                        ...
                        <q-avatar 
                            size="24px" 
                            v-else 
                            :style="{'background-color':this.randomColor(),'border':'solid 1px white'}" 
                            text-color="white" 
                            class="text-weight-bold"
                        >
                            <div class="absolute-center">{{props.row.fullname.split('')[0].toUpperCase()}}</div>
                        </q-avatar>
                    </q-td>
        ...
        <q-dialog v-model="prompt" persistent >
            <q-card style="min-width: 350px" dark>
                ...
                <q-card-section class="q-pt-none">
                    <q-input filled  dense v-model="user_data.fullname" autofocus  label="Fullname" dark clear/>
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <q-input filled  dense v-model="user_data.employee_id" autofocus label="Employee ID" dark clear/>
                </q-card-section>

                <q-card-section class="q-pt-none" v-if="user_data.id == ''">
                    <q-input filled  dense v-model="user_data.password" :type="isPwd ? 'password' : 'text'" autofocus label="Password" dark clear>
                        <template v-slot:append>
                            <q-icon
                ...
                <q-card-section class="q-pt-none">
                    <q-file filled dense v-model="user_data.profile" label="Profile Image" stack-label dark clear/>
                </q-card-section>

                <q-card-actions align="right" class="bg-blue-grey-8">
                    <q-btn glossy size="sm" rounded label="Cancel" v-close-popup color="negative" />
                    <q-btn glossy size="sm" v-if="user_data.id == ''" rounded label="Submit" @click="createUser" color="positive" />
                    <q-btn glossy size="sm" v-else rounded label="Update" @click="updateUser" color="primary" />
                </q-card-actions>
            </q-card>
        </q-dialog>
    ...
    methods: {
        resetPassword(props){
            this.$q.dialog({
                ...
                axios.post(this.apiUrl+'user/resetpass',formData,{
        ...
        updateUser(){
            ...
            axios.put(this.apiUrl+'user/update',formData,{
                headers: { 'Content-Type':'application/json'}
            }).then((res) => {
        ...
        createUser(){
            ...
            axios.post(this.apiUrl+'add-user',formData,{
                headers: {'Content-Type': 'application/json'}
            }).then((res) => {
                var myRes = res.data
                if(myRes.status == 200){
        ...
        getRowsNumberCount(filter) {
            axios.post(this.apiUrl+"users/count",
            { filter: filter,},
            {
                headers: {
                "Content-Type": "multipart/form-data",
